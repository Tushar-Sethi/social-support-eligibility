...
if "pending_chat_response" not in st.session_state:
    st.session_state.pending_chat_response = None

# --------------------------------------------------------------------
# Function to handle LLM response in a separate thread
# --------------------------------------------------------------------
def process_chat_input(user_input, state, app_agent):
    return app_agent.run(state, user_input)
...
if submit:
    ...
    if missing_fields:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(process_chat_input, initial_prompt, st.session_state.state, st.session_state.app_agent)
            st.session_state.pending_chat_response = future
    else:
        # 4. All required fields are present: parse documents and store results
        # 4a. Bank Statement Parsing
        try:
            st.session_state.df_bank = process_bank_statement(bank_statement_file)
        except Exception as e:
            st.session_state.df_bank = None
            st.session_state.validation_errors = [f"Failed to parse Bank Statement: {e}"]

        # 4b. Resume Parsing
        try:
            st.session_state.df_resume = process_resume(resume_file)
        except Exception as e:
            st.session_state.df_resume = None
            if st.session_state.validation_errors:
                st.session_state.validation_errors.append(f"Failed to parse Resume: {e}")
            else:
                st.session_state.validation_errors = [f"Failed to parse Resume: {e}"]

        # 4c. Assets & Liabilities Parsing
        try:
            st.session_state.df_assets_liabilities = process_assets_liabilities(assets_liabilities_file)
        except Exception as e:
            st.session_state.df_assets_liabilities = None
            if st.session_state.validation_errors:
                st.session_state.validation_errors.append(f"Failed to parse Assets & Liabilities: {e}")
            else:
                st.session_state.validation_errors = [f"Failed to parse Assets & Liabilities: {e}"]

        # 4d. Credit Report Parsing
        try:
            st.session_state.df_credit_report = process_credit_report(credit_report_file)
        except Exception as e:
            st.session_state.df_credit_report = None
            if st.session_state.validation_errors:
                st.session_state.validation_errors.append(f"Failed to parse Credit Report: {e}")
            else:
                st.session_state.validation_errors = [f"Failed to parse Credit Report: {e}"]
        ...
        if not validation_ok:
            ...
            try:
                ...
                st.session_state.df_employment_metrics = None
                st.session_state.validation_errors = [f"Failed to parse Employment History: {e}"]
            ...
            try:
                ...
                st.session_state.df_wealth_metrics = None
                st.session_state.validation_errors = [f"Failed to compute Assets/Liabilities metrics: {e}"]
            ...
            try:
                ...
                st.session_state.prediction_value = None
                st.session_state.trained_pipeline = None
                st.session_state.validation_errors = [f"Prediction failed: {e}"]
            ...
            if st.session_state.trained_pipeline is not None and st.session_state.prediction_value is not None:
                ...
                try:
                    ...
                    st.session_state.model_explanation = ""
                    if st.session_state.validation_errors:
                        st.session_state.validation_errors.append(f"Failed to generate explanation: {e}")
                    else:
                        st.session_state.validation_errors = [f"Failed to generate explanation: {e}"]
            ...
            st.session_state.state = {
                ...
                "validation_errors": st.session_state.validation_errors or [],
...
if st.session_state.submitted:
    # 3a) Display all parsing & model outputs
    st.markdown("---")

    # 3a.1) If there were validation errors, show them
    if st.session_state.validation_errors:
    ...
    # Check for completed pending response
    if st.session_state.pending_chat_response is not None and isinstance(st.session_state.pending_chat_response, concurrent.futures.Future):
        if st.session_state.pending_chat_response.done():
            response = st.session_state.pending_chat_response.result()
    ...
    # Use st.chat_input for follow-ups
    if user_input := st.chat_input("Ask a question about your application"):
        # Save the user’s message
        st.session_state.chat_history.append(("You", user_input))
        st.chat_message("user").write(user_input)

        # Start processing the response in a separate thread
        chat_id = str(uuid.uuid4())
        st.session_state.pending_chat_id = chat_id
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(process_chat_input, user_input, st.session_state.state, st.session_state.app_agent)
            st.session_state.pending_chat_response = future
        st.experimental_rerun()